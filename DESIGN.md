...
- **核对页（专属）:** 画布吃满宽度。OCR 矩形框和编号钉分开关；拖动画布时框暂时隐藏。工具条可收缩。拖动画布（不要地球图标）、滚轮/按钮缩放，点核对窗序号后局部放大该框。核对窗钉在整页右上（不叠在图里），可拖、四角缩放、可收起：左列当前字段（Excel 应印 / 稿上 OCR），右列疑点列表。结论在页头签字旁填写。点字段，图上跟到这一条。工艺说明 / 颜色要求 / 版本号不进机审。
