...
- **Scale（自适应，不要太小也不要太大）:** 辅助 ≥13px，正文 ≥16px，页名 28px，侧栏项 17px。控件高度 ≥40px
...
- **Approach:** 左玻璃侧栏 + 右工作台。不是顶栏。
- **展开侧栏:** 280px，圆角 28px，`backdrop-filter: blur(24px)`，底 `rgba(255,255,255,0.58)`，1px `#E7E2EC`，浅紫阴影。
...
- **侧栏项:** 248×56（展开时撑满内宽），圆角 12px，图标 22px + 17px 文案。Hover 浅灰块；Selected **2px** `#805898` 框 + `purple-soft` 底 + 紫字。不要紫胶囊填满。
- **顺序:** 审稿台 → 打样台 → 历史记录 → 设置。
- **账户行:** 左下角。飞书头像 28px + `花名（真名）`，例如 `天元（魏炜）`。来自登录态，不要写死。点开可退出。无花名时只显示真名。
