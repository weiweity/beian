...
- **打样台:** 同一玻璃语言，一个大井只交 `.ai`。不要做成审稿的双井。不要再收平面 PDF。
- **核对页（专属）:** 左画布叠 OCR 矩形框（紫框已命中 / 黄框待核对），编号钉在框中心；没有 bbox 或页宽高未知就不画假钉。右 400px 一对一检视（当前字段、Excel 应印、稿上 OCR、定位、一致/有错/忽略、疑点列表、写下结论）。点字段，图上跟到这一条。
- **等待卡:** 狐狸球（正圆身体 + logo-mark + 绕轨/彩环）+ 大号 ETA + 阶段胶囊 + Apple 式进度条。三种：对照中 / 对红中 / 打样中。排队只写「前面还有 N 单」或「就快轮到」，不准写 40 秒。跑起来 ETA 用大约：对照/对红约 40 秒，打样约 4 分钟；≥60 秒写分钟。离开后再进审稿台看板或打样台，仍能看到阶段；打样台自动跟正在跑的那单。**不要做假取消**——后端取不消对照/Blender 就不要写「取消」。
...
| 她提交 | 「已记录你的结论，不是系统过审」 |
| 打样中 | 等待卡「打样中」。排队写「前面还有 N 单」；跑起来写「大约还要 4 分钟」。本机 Blender。白底不要带尺寸标注再交备案 |
