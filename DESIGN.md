...
- **核对页（专属）:** 画布吃满宽度。OCR 矩形框和编号钉分开关；拖动画布时框暂时隐藏。工具条可收缩。拖动画布（不要地球图标）、滚轮/按钮缩放，点核对窗序号后局部放大该框。核对窗浮在整页最上面（含侧栏），进页展开并叠在左侧栏上，可随意拖，不挤页图。窗 z 最高可盖画布，但夹住不盖页头签字。收起/展开 280ms Gentle；边框上下左右和四个斜角都能缩放。左列当前字段：疑点/错误点（有 miss 才出现）在 Excel 应印上面，然后稿上 OCR。右列疑点列表。结论在页头签字旁填写。点字段，图上跟到这一条。工艺说明 / 颜色要求 / 版本号不进机审（只认字段名开头）。
