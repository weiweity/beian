...
- **打样台:** 同一玻璃语言，一个大井只交 `.ai`。不要做成审稿的双井。不要再收平面 PDF。打样台只交稿和列单；点进度或已出图进单独打样单（白底正面+侧面 / 反面+侧面带阴影、GLB 可全屏截图、可下载 PPT）。
- **核对页（专属）:** 画布吃满宽度。OCR 矩形框和编号钉分开关；拖动画布时框暂时隐藏。工具条可收缩。拖动画布（不要地球图标）、滚轮/按钮缩放，点核对窗序号后局部放大该框。核对窗钉在页头下面靠右（钉钉/飞书检视栏，不盖签字），可拖；边框上下左右和四个斜角都能缩放。左列当前字段（Excel 应印 / 稿上 OCR），右列疑点列表。结论在页头签字旁填写。页头压在核对窗上面，签字点得到。点字段，图上跟到这一条。工艺说明 / 颜色要求 / 版本号不进机审（只认字段名开头）。
...
| 待她判 | 核对页：图上真框 + 钉在框中心 + 页头下面靠右核对窗（边框八向缩放，不盖签字）；钉和框分开关；没有位置就不画假钉；OCR 不清必须写「待人工确认」 |
| 她提交 | 「已记录你的结论，不是系统过审」 |
...
| 打样台可跑 | 选稿 → 跑 worker → 点进打样单看/下载白底（正面+侧面、反面+侧面）和 GLB（有 Node 才有 PPT）；失败写清缺 Blender、缺刀线或套不上已登记刀模。没有 Node 不把整单判失败 |
