...
- **等待卡:** 狐狸球（正圆身体 + logo-mark + 绕轨/彩环）+ 大号 ETA + 阶段胶囊 + Apple 式进度条。三种：对照中 / 对红中 / 打样中。排队只写「前面还有 N 单」或「就快轮到」，不准写 40 秒。跑起来 ETA 用大约：对照/对红约 40 秒，打样约 4 分钟；≥60 秒写分钟。**不要做假取消**——后端取不消对照/Blender 就不要写「取消」。
