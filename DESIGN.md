...
- **核对页（专属）:** 左画布叠 OCR 矩形框（紫框已命中 / 黄框待核对），编号钉在框中心；没有 bbox 或页宽高未知就不画假钉。滚轮/按钮缩放，点疑点序号后局部放大该框。左右拖条改分栏宽度（默认右 400px）。右栏一对一检视（当前字段、Excel 应印、稿上 OCR、定位、一致/有错/忽略、带序号的疑点列表可滚动、写下结论）。点字段，图上跟到这一条。
...
| 待她判 | 核对页：图上真框 + 钉在框中心 + 右侧一对一；没有位置就不画假钉；OCR 不清必须写「待人工确认」 |
| 她提交 | 「已记录你的结论，不是系统过审」 |
