...
- **打样台:** 同一玻璃语言，一个大井只交 `.ai`。不要做成审稿的双井。不要再收平面 PDF。打样台只交稿和列单；点进度或已出图进单独打样单。打样台和打样单主区白底深字（`#ffffff`，不是紫雾台；深色主题也是）。打样单一屏三列：正面+侧面、反面+侧面、GLB；下载是 figcaption 右侧 44 玻璃圆钮（iOS 27 square.and.arrow.down），不压在产品像素上。GLB 中性环境、曝光跟静帧，接触阴影。已经出过的白底图要重新打样才会变。打样单 PDF 槽按源图比例 contain，不拉长。
- **核对页（专属）:** 画布吃满宽度。OCR 矩形框和编号钉分开关；拖动画布时框暂时隐藏。工具条可收缩。拖动画布（不要地球图标）、滚轮/按钮缩放，点核对窗序号后局部放大该框。核对窗浮在整页最上面（含侧栏），进页展开并叠在左侧栏上；页图和放大/钉工具落在窗右侧剩余区，不与窗重叠。窗 z 最高可盖画布，但夹住不盖页头签字。收起/展开 280ms Gentle；可拖；边框上下左右和四个斜角都能缩放。左列当前字段：疑点/错误点（有 miss 才出现）在 Excel 应印上面，然后稿上 OCR。右列疑点列表。结论在页头签字旁填写。点字段，图上跟到这一条。工艺说明 / 颜色要求 / 版本号不进机审（只认字段名开头）。
...
| 待她判 | 核对页：图上真框 + 钉在框中心 + 整页最上面核对窗（进页展开叠在左侧栏；页图和放大/钉进窗右侧剩余区；窗可盖画布，夹住不盖页头签字；收起有动画，可拖，边框八向缩放）；钉和框分开关；没有位置就不画假钉；OCR 不清必须写「待人工确认」 |
| 她提交 | 「已记录你的结论，不是系统过审」 |
...
| 打样台可跑 | 选稿 → 跑 worker → 点进打样单一屏三图：正面+侧面、反面+侧面、GLB。主区白底。下载是图下说明行右侧 44 玻璃圆钮，不压产品。PDF 槽 contain。PPT 用两张白底写 OOXML，不依赖 Node；写不出才试演示文稿运行时。白底仍合成 PDF（页底先铺白；pymupdf 写不出且还没落盘才用已装 Pillow，不盖掉已写成的文件）。失败写清缺 Blender、缺刀线或套不上已登记刀模 |
