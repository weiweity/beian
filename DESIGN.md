...
| 待她判 | 核对页：图上真框 + 钉在框中心 + 右侧一对一；隐藏钉只藏编号；没有位置就不画假钉；OCR 不清必须写「待人工确认」 |
| 她提交 | 「已记录你的结论，不是系统过审」 |
| 打样中 | 等待卡「打样中」。排队写「前面还有 N 单」；跑起来写「大约还要 4 分钟」，阶段芯片「出图 / 打样 / 导出」。本机 Blender。白底不要带尺寸标注再交备案。离开再回看板或打样台仍能看到阶段 |
| 打样台可跑 | 选稿 → 跑 worker → 下载白底/PPT/GLB；失败写清缺 Blender、缺刀线或套不上已登记刀模 |
