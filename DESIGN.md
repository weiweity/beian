...
- **打样台:** 同一玻璃语言，一个大井只交 `.ai`。不要做成审稿的双井。不要再收平面 PDF。打样台只交稿和列单；点进度或已出图进单独打样单。打样台和打样单主区白底深字（`#ffffff`，不是紫雾台；深色主题也是）。打样单一屏三列等高：正面+侧面、反面+侧面、GLB；每张图右上角 44 玻璃圆钮下载。页头右上字样「下载+PPT」。点下载出底部玻璃条提示，不挡操作。GLB 全屏后水平垂直居中。GLB 中性环境、曝光跟静帧，接触阴影。已经出过的白底图要重新打样才会变。打样单 PDF 槽按源图比例 contain，不拉长。
...
| 打样台可跑 | 选稿 → 跑 worker → 点进打样单一屏三图：正面+侧面、反面+侧面、GLB。主区白底。每张图右上角 44 玻璃圆钮下载；页头「下载+PPT」。点下载底部提示，不挡操作。GLB 全屏居中。PDF 槽 contain。PPT 用两张白底写 OOXML，不依赖 Node；写不出才试演示文稿运行时。白底仍合成 PDF（页底先铺白；pymupdf 写不出且还没落盘才用已装 Pillow，不盖掉已写成的文件）。失败写清缺 Blender、缺刀线或套不上已登记刀模 |
...
| 2026-08-25 | 打样图右上下载 | 三图等高对齐；下载在图右上角；页头「下载+PPT」；底部玻璃条提示不挡操作；GLB 全屏居中 |
