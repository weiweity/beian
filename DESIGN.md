...
- **窄屏:** ≤1024 默认折叠侧栏（折叠轨 `39:1116`）。390 一列看板。展开不挤主区：≤720 用 Figma `32:1081` 玻璃层盖在主区上，点遮罩收回。台签不拆字。液态玻璃抽屉盖在核对页上时改更实雾面并关掉色散边，避免稿面字糊进侧栏。
...
| 2026-08-23 | 降低透明度 | 系统 `prefers-reduced-transparency` 时毛玻璃/液态玻璃（含深色）blur 降到 12px，液态玻璃底更实，色散边和折叠钮玻璃关掉 |
| 2026-08-23 | 窄屏液态抽屉 | ≤720 展开盖核对页时液态玻璃改更实雾面，避免稿面字糊进侧栏 |
