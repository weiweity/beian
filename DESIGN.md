...
- **核对页（专属）:** 画布吃满宽度。OCR 矩形框和编号钉分开关；拖动画布时框暂时隐藏。工具条可收缩。拖动画布（不要地球图标）、滚轮/按钮缩放，点核对窗序号后局部放大该框。核对窗钉在整页右上（不叠在图里），可拖、四角缩放、可收起：左列当前字段（Excel 应印 / 稿上 OCR），右列疑点列表。结论在页头签字旁填写。页头压在核对窗上面，签字点得到。点字段，图上跟到这一条。工艺说明 / 颜色要求 / 版本号不进机审（只认字段名开头）。
- **等待卡:** Generating 圆盘字母（对照中 / 对红中 / 打样中）自适应尺寸 + 大号 ETA + 阶段胶囊 + Apple 式进度条。三种：对照中 / 对红中 / 打样中。排队只写「前面还有 N 单」或「就快轮到」，不准写 40 秒。跑起来 ETA 用大约：对照/对红约 40 秒，打样约 4 分钟；≥60 秒写分钟。离开后再进审稿台看板、打样台或历史记录，仍能看到阶段；打样台自动跟正在跑的那单。点历史记录里进行中的单，仍进等待卡。**不要做假取消**——后端取不消对照/Blender 就不要写「取消」。
