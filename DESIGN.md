...
- **核对页（专属）:** 画布吃满宽度。OCR 矩形框和编号钉分开关；拖动画布时框暂时隐藏。工具条可收缩。拖动画布（不要地球图标）、滚轮/按钮缩放，点核对窗序号后局部放大该框。核对窗浮在整页最上面（含侧栏），默认定在页头下面靠右不盖签字，可拖到侧栏；边框上下左右和四个斜角都能缩放。左列当前字段（Excel 应印 / 稿上 OCR，含未读到的短语），右列疑点列表。结论在页头签字旁填写。点字段，图上跟到这一条。工艺说明 / 颜色要求 / 版本号不进机审（只认字段名开头）。
...
| 待她判 | 核对页：图上真框 + 钉在框中心 + 整页最上面核对窗（默认定页头下，可拖到侧栏，边框八向缩放）；钉和框分开关；没有位置就不画假钉；OCR 不清必须写「待人工确认」 |
| 她提交 | 「已记录你的结论，不是系统过审」 |
