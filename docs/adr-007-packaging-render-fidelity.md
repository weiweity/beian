...
- 实现快照：RF-00 / RF-01 / RF-02 已交 Code/L0（RF-02.3 收口磁盘执行边界与私有执行快照/nonce）。新任务消费 persistable render plan 并写入四项顶层 identity / fingerprint token / resolved job / result；Blender 启动前校验磁盘 payload 与派生 flat render。所有非 cache Blender 任务从当前内存 job 生成私有执行快照并注入本轮 nonce；非 V2 只是命令行诊断路径，不是网页产品通道。只有已知 pre-RF02 V2 作业可合成 `compat-legacy-v0`，且历史最小 render 键必须完整匹配。同步异常全量回滚（含删除本轮新建 optional 输出）；进程崩溃/断电级原子 current 仍是 RF-03。`0.23.0.0` 候选已获批冻结 [Mac 合成现状 baseline](../workers/packaging/fixtures/render-quality/baselines/README.md)，仅用于 legacy 默认身份回归；PNG 按解码像素比较，GLB 文件哈希及缺失指标仍严格拒绝。Windows、真实稿与人工质量批准仍开放。RF-06 材质、RF-07 显式棚光/三变换对照与 RF-08 投影采样只在诊断 registry/候选 profile 中生效，未批准前保持 Standard 与 `minimum-floor-v1` 默认，不得称生产画质已改善。RF-10 把 runtime / fixture regression / `human_acceptance` 分成独立字段；真实任务上 fixture 为 `not-run`，机器通过不会写成人工通过或 `production_ready`。RF-11 smoke 只写 `RUNNER_TEMP`，未解析到 Blender 时跳过不回滚。`0.21.46.0` 发版 L1 已过且杭州真跑冒烟约 62s，质量门杭州实跑已满足；不等于正式 baseline、L2 或 UAT 已完成。

## 1. 结论先行

> 2026-09-08 RF-11 杭州实跑：`0.21.46.0` hangzhou-release `34191398011`（SHA `3fcfea75`）真跑 `WINDOWS_BLENDER_CONTRACT_SMOKE ok`，墙钟约 62s；发版 `TimeoutMs` 为 90000；wrapper 为 `PROC_THREAD_ATTRIBUTE_JOB_LIST` + `CREATE_SUSPENDED` + `KILL_ON_JOB_CLOSE`。入 Job 失败会回滚，该次成功路径走通；超时杀树分支未走到。未改生产 registry、Standard 或正式 baseline。L2/UAT 见 [TODOS.md](../TODOS.md)。

> 2026-09-08 RF-11 冻结（代码切片）：`0.21.45.0` hangzhou-release `34186013159` 真跑 Blender 5.2 合同冒烟约 63s。`0.21.46.0` 把发版 `TimeoutMs` 冻结为 90000，并改 Job Object 杀树。该条记录冻结当时的代码意图；杭州 Job Object 成功路径证据见上一条。
...
| RF-10 | P4 / XL | RF-05–RF-09 | `quality_layers.py`、quality eval CLI、generation worker / Hono 分层字段、合同测试 | 已合 `0.21.44.0` Code/普通 L0：runtime/fixture/human 三层分开；`baseline_mismatch` 只失败夹具门；`--update-baseline` 必须显式；真实任务 fixture 为 `not-run`；`human_acceptance` 独立 pending，`production_ready` 保持 false。正式 baseline、L2/UAT 见 TODOS |
| RF-11 | P4 / L | RF-03, RF-05, RF-10 | `blender-contract-smoke.ps1` + `blender_contract_smoke.py`、`release.ps1`、合同测试 | 已合 `0.21.44.0` Code/普通 L0。`0.21.45.0` 杭州真跑 Blender 5.2 约 63s。`0.21.46.0` hangzhou-release `34191398011` 真跑约 62s，`TimeoutMs 90000`，Job Object 成功路径走通。缺已解析 Blender 时跳过不回滚。L2/UAT 见 TODOS |
...
- [x] **T10（P1，human: ~2d / Grok: ~4h）— UI clarity — 让 card 原位升级 full 并释放资源**
...
- [x] **T11（P1，human: ~3d / Grok: ~6h）— quality — 实现三层质量门与 baseline identity（Code/普通 L0）**
  ...
  - Verify: Code/普通 L0 已合 `0.21.44.0`：分字段记录 runtime hard、fixture hard、human warning；mismatch 失败夹具门；baseline 只能显式更新。正式 approved baseline、L2/UAT 仍开放，见 TODOS「质量门与回归矩阵」。
- [x] **T12（P1，human: ~2d / Grok: ~4h）— release — 接入 Windows Blender 发布冒烟（Code/普通 L0 接线）**
  ...
  - Verify: Code/普通 L0 已合 `0.21.44.0`：smoke 在 drain 内、只写 `RUNNER_TEMP`，复用 quality eval 与 generation GLB/hash 门。`0.21.46.0` hangzhou-release `34191398011` 真跑约 62s，`TimeoutMs 90000`，Job Object 成功路径走通。缺已解析 Blender 时跳过不回滚。L2/UAT 见 TODOS「质量门与回归矩阵」。
...
- [ ] **T14（P3，human: ~1d + 人工评审 / Grok: ~2h support）— acceptance — 执行 L1/L2/UAT**
  ...
  - Verify: 真实 carton 旧/新盲评、合同 hash、评审人和刘籽烨完整流程；真稿/输出不进 Git。发版 L1 与质量门杭州实跑已过不等于本项 L2/UAT。
