...
- 日期：2026-09-04
- 状态：PARTIAL IMPLEMENTATION — `/plan-eng-review` 已完成，D1–D8 已锁定；RF-00～RF-08 见既有切片（诊断 registry 未进生产默认）；RF-09 浏览器分层升级已完成本地实现与合成验证；RF-10/RF-11 已合 `main` `0490e46` / `0.21.44.0`（Code/普通 L0）。`0.21.45.0` 发版 L1 已过且杭州真跑 Blender 5.2 冒烟约 63s；`0.21.46.0` 冻结 90 秒并用 Job Object 杀树，质量门杭州实跑须下次发版证明。正式 approved baseline、生产注册、L2/UAT 见 [TODOS.md](../TODOS.md)，不要用发版 L1 勾它们
...
硬超时已按杭州同机实测冻结：`0.21.45.0` run 34186013159 墙钟约 63s，`0.21.46.0` 起 `release.ps1` 传入 `-TimeoutMs 90000`，JSON `timeout_budget_status=hangzhou_frozen_90s`。超时、Blender 缺失、版本不符、contract/hash/GLB/generation 任一 hard gate 失败，都保持 drain 并走现有 release recovery。该 smoke 只证明 Windows 运行链可用，不证明真实感、字体清晰度或真实稿 L2/UAT。质量门杭州实跑（Job Object 杀树在杭州再证）仍未勾。
...
| RF-11 | P4 / L | RF-03, RF-05, RF-10 | `blender-contract-smoke.ps1` + `blender_contract_smoke.py`、`release.ps1`、合同测试 | 已合 `0.21.44.0` Code/普通 L0。`0.21.45.0` 杭州真跑 Blender 5.2 约 63s。`0.21.46.0` 冻结发版 `TimeoutMs 90000`，wrapper 用 Job Object 杀树。缺已解析 Blender 时跳过不回滚。质量门杭州实跑须下次发版证明 Job Object，见 TODOS |
...
- [x] **T12（P1，human: ~2d / Grok: ~4h）— release — 接入 Windows Blender 发布冒烟（Code/普通 L0 接线）**
  ...
  - Verify: Code/普通 L0 已合 `0.21.44.0`：smoke 在 drain 内、只写 `RUNNER_TEMP`，复用 quality eval 与 generation GLB/hash 门。`0.21.45.0` 杭州真跑约 63s。`0.21.46.0` 冻结 90s 并改 Job Object。缺已解析 Blender 时跳过不回滚。质量门杭州实跑须下次发版证明，见 TODOS「质量门与回归矩阵」。
