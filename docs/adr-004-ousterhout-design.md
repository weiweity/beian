...
`apps/web/server/src/auth.ts` 与 `index.ts` 应隐藏飞书 OAuth、账号目录、会话 cookie 和角色解析。UI 只消费当前身份与服务端返回的能力集合，不直接用角色名复制授权规则。授权只认不可变的飞书 `open_id` 与本机 `users.json` 的显式 `admin/reviewer/viewer` 角色绑定；姓名和花名不参与授权。共享读取、动作能力和对象所有权是三层不同合同：团队打样单允许所有已登录角色读取；`confirm_structure` 授予 `admin` / `reviewer` / `viewer`；删除仍要求对象所有者或管理员。每次请求重新校验账号仍存在、未停用且角色合法，使删除、停用和降权立即作用于已有会话；目录不可读、重复 `open_id` 或未知角色一律失败关闭。账号目录故障使用有类型的 403/503 边界：本机登录 API 保留 HTTP 状态，飞书 OAuth 回调把 403 映射为 `forbidden`、把 503 映射为 `failed`，不能再靠消息正则误分，也不能泛化成 500。显示名会话同时绑定 loopback 与当前登录模式，切到公网或关闭显示名登录时从内存和持久化文件撤销；退出只依赖 token，不依赖目录可读性。
