...
6. 对没有对象级语义的历史 AI，Illustrator 只盘点未分配的“描边且无填充”图层，形成有上限、绑定当前源稿 SHA-256 的输入候选；普通 OCG、图层名、颜色和白色区域都不能自动选中结构层。当前 worker 只落盘候选，Hono/API/UI 尚未开放选层入口；在该边界完成候选成员校验之前，候选不能进入生产结构。后续只有管理员从本次候选中明确选择后，Illustrator 才提取所选层的路径作为不可信线稿。直线保持原端点；三次贝塞尔曲线由唯一的 ES3 helper 自适应细分，并把容差、原路径引用、段数和 `bezier_flatten` repair 写入 `source.geometry`。Mac AppleScript 与 Windows Agent 都在执行前把这份固定 helper 内联到运行 JSX，避免相对 include、当前目录和系统代码页产生平台分叉。超过候选层、单段、单路径或全稿预算时失败关闭。
...
- 无显式对象语义的旧 AI 当前只能盘点待人工选择的纯描边图层候选；公开选层入口尚未开放。后续选择必须绑定当前源稿并校验候选成员，且所选线稿仍要通过单位、完整盒身环和上下封口验证，否则进入 `review_required` / `unsupported`。
