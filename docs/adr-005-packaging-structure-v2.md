...
6. 对没有对象级语义的历史 AI，Illustrator 只盘点未分配的“描边且无填充”图层，形成有上限、绑定当前源稿 SHA-256 的输入候选；普通 OCG、图层名、颜色和白色区域都不能自动选中结构层。候选盘点可附带独立的 `illustrator-layer-preview/1` 建议性快照：沿同一画板左上角坐标保留真实直线和三次贝塞尔控制点，单层最多 512 路径；刀线候选与填色工艺板各有 5000 路径 / 20000 点预览预算，互不占用。Illustrator 先盘点描边层，预算再装不下一条两点路径时不再读 `PathPoints`，第二遍只扫第一遍剩下的项盘点填色工艺板。Hono 再校验页尺寸、候选成员、坐标，并按种类分开计数预算。快照只驱动网页移入/勾选后的涂亮预览：未知或重复的涂亮层、坏掉的工艺板单独跳过；超大或非法几何才关掉整份预览，但不破坏候选身份，也永远不能进入拓扑或改变 `cut/crease` 语义。Hono/API/UI 只允许管理员提交本次详情返回的候选 ID（最多 16 个），服务端再次校验 `confirm_structure`、管理员身份、源稿身份、候选成员和单任务串行后，才生成新的源稿绑定清单并重跑同一作业；不得接受客户端手填图层名。非管理员详情省略 `structure_input`。填色工艺板（如烫雅银）只进入预览，不能当作 cut/crease。网页对唯一「上刀线」（或全单只剩一层「刀线」）可默认提交已盘点候选 ID，候选里唯一「印刷」可一并带上，失败对籽烨结案为打样失败，选层只给管理员；旁边还有「折线」的「刀线」仍预勾。提交仍按候选 ID 校验，不按图层名自动接受 cut/crease 语义。Illustrator 随后提取所选层的路径作为不可信线稿。直线保持原端点；三次贝塞尔曲线由唯一的 ES3 helper 自适应细分，并把容差、原路径引用、段数和 `bezier_flatten` repair 写入 `source.geometry`。Mac AppleScript 与 Windows Agent 都在执行前把这份固定 helper 内联到运行 JSX，避免相对 include、当前目录和系统代码页产生平台分叉。超过候选层、单段、单路径或全稿预算时失败关闭。
...
9. 候选模型是“四个连续盒身面 + 上下两个封口组件”。盒身候选必须四边都有来源线，内部尺寸线、标注线或正文描边不会把物理盒面切成两个；缺一条外边的三边框只能作为封口候选，不能进入盒身环。完整闭合且尺寸接近 footprint 的单片盖板仍优先。对 FEFCO 0201/RSC 一类相向对开摇盖，两个成员必须连接到相对盒身面，每片覆盖 45%–55%，覆盖率总和位于 94%–102%，且在成盒 footprint 上的几何并集至少为 94%；超过工艺缝范围的重叠因为缺少可信叠放顺序而失败关闭。单个短防尘翼、插舌、正交侧翼或由交叉标注线构造的开放盒身仍会被拒。确认后每个成员按自己的共享折线独立确定方向和仿射变换；原始 PDF 未绘制区域与物理未覆盖区域的 Alpha 都保留为 0，不得拉伸或填成白色。Blender 以二值 Alpha Mask 覆盖在显式 `render.substrate_rgba` 纸板核心之上。最终 GLB 先从 BIN `bufferView` 解码六张内嵌 8-bit RGBA PNG，与源资产逐像素及 Alpha 平面对照；再回读导出物，按世界坐标核对六面 UV 角点、方向和镜像。唯一不透明 Core 必须是边均双邻接的单一闭合连通流形，尺寸、表面积、体积及六个实体边界都接近完整盒体。图片同名、材质同名、一个退化 primitive 或六个断开的边界小面片都不能通过。未知基材明确默认白卡 `[1,1,1,1]`；牛皮纸或深色纸板必须由渲染配置显式给值，不能从印刷颜色猜测。成盒 `width/depth/height` 只由四个盒身面求解。若同稿同时存在外盒与白色内衬，只有各自结构完整时才分别成为候选并按成盒体积排序；管理员通过真实展开图的视觉翻页选中目标候选，系统仍不读取颜色作判断。
10. 每个候选在返回页面前，使用与最终确认完全相同的决策引擎枚举四个盒身面和 0/90/180/270° 阅读方向。只有能唯一推导 front/right/back/left/top/bottom、折叠关系和六面 artwork transform 的锚点才进入 `valid_anchors`。预检还为每个正面写入 `preferred_quarter_turns`：选择使规范化盒身环向物理右侧推进的可用方向；这是一条从已确认几何得出的确定性建议，不读取颜色、文字或文件名。唯一可确认正面且带 `preferred_quarter_turns` 时，网页路径由服务端自动确认并继续出图，不再停在 A–D。多面时页面只让已登录且具备 `confirm_structure` 的账号选择正面并自动提交该建议，确认时仍再次校验当前源稿、structure hash、basis 与锚点，防止旧页面提交已失效序号。管理员对已出图且只有一套公开盒型的单可再提交正面，只重跑 Blender。旧记录缺 `valid_anchors`、缺推荐方向或缺与当前结构一致的 `structure_hash` 时整个候选失败关闭并要求重新识别，不能回退成任意方向。
...
- 操作者只提供业务判断 `front_face_id`。唯一可确认正面时服务端自动提交该锚点。若有多套安全候选，只通过真实展开图的上一张/下一张选择，不暴露尺寸与封口算法；页面把候选 `proposal_id` 和预检给出的 `preferred_quarter_turns` 作为内部锚点一并提交。服务端合同仍是 `proposal_id + front_face_id + quarter_turns`，不逐面填写六个角色，也不降低最终预检。管理员对已出图且只有一套公开盒型的单可再提交换正面，只重跑 Blender。
...
- 无显式对象语义的旧 AI 先盘点待人工选择的纯描边图层候选；只有管理员可在打样单详情中多选本稿候选并重跑同一作业。非管理员详情省略图层候选，组不成花盒时看板写打样失败。选择必须绑定当前源稿并校验候选成员，且所选线稿仍要通过单位、完整盒身环和上下封口验证，否则继续进入 `review_required` / `unsupported`。
