...
6. 对没有对象级语义的历史 AI，Illustrator 只盘点未分配的“描边且无填充”图层，形成有上限、绑定当前源稿 SHA-256 的输入候选；普通 OCG、图层名、颜色和白色区域都不能自动选中结构层。候选盘点可附带独立的 `illustrator-layer-preview/1` 建议性快照：沿同一画板左上角坐标保留真实直线和三次贝塞尔控制点，单层最多 512 路径；刀线候选与填色工艺板各有 5000 路径 / 20000 点预览预算，互不占用。Illustrator 先盘点描边层，预算再装不下一条两点路径时不再读 `PathPoints`，第二遍只扫第一遍剩下的项盘点填色工艺板。Hono 再校验页尺寸、候选成员、坐标，并按种类分开计数预算。快照只驱动网页移入/勾选后的涂亮预览：未知或重复的涂亮层、坏掉的工艺板单独跳过；超大或非法几何才关掉整份预览，但不破坏候选身份，也永远不能进入拓扑或改变 `cut/crease` 语义。Hono/API/UI 只允许已登录且具备 `confirm_structure` 的账号提交本次详情返回的候选 ID（最多 16 个），服务端再次校验权限、源稿身份、候选成员和单任务串行后，才生成新的源稿绑定清单并重跑同一作业；不得接受客户端手填图层名。填色工艺板（如烫雅银）只进入预览，不能当作 cut/crease。网页对唯一「上刀线」（或全单只剩一层「刀线」）可默认提交已盘点候选 ID，候选里唯一「印刷」可一并带上，失败才露出选层；旁边还有「折线」的「刀线」仍预勾。提交仍按候选 ID 校验，不按图层名自动接受 cut/crease 语义。Illustrator 随后提取所选层的路径作为不可信线稿。直线保持原端点；三次贝塞尔曲线由唯一的 ES3 helper 自适应细分，并把容差、原路径引用、段数和 `bezier_flatten` repair 写入 `source.geometry`。Mac AppleScript 与 Windows Agent 都在执行前把这份固定 helper 内联到运行 JSX，避免相对 include、当前目录和系统代码页产生平台分叉。超过候选层、单段、单路径或全稿预算时失败关闭。
...
- 操作者只提供业务判断 `front_face_id`。若有多套安全候选，只通过真实展开图的上一张/下一张选择，不暴露尺寸与封口算法；页面把候选 `proposal_id` 和预检给出的 `preferred_quarter_turns` 作为内部锚点一并提交。服务端合同仍是 `proposal_id + front_face_id + quarter_turns`，不逐面填写六个角色，也不降低最终预检。
...
- 无显式对象语义的旧 AI 先盘点待人工选择的纯描边图层候选；已登录且具备 `confirm_structure` 的账号可在打样单详情中多选本稿候选并重跑同一作业。选择必须绑定当前源稿并校验候选成员，且所选线稿仍要通过单位、完整盒身环和上下封口验证，否则继续进入 `review_required` / `unsupported`。
