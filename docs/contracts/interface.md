...
- 打样台在 2026-08-31 阶段不对业务开放；后续开放须有明确决定，不能按日期自动解禁。打样台和打样单主区白底深字（不是紫雾台，深色主题也是）；有地面图的新单走 grounded 成片（台面 `rgb(228,228,232)`），布局仍是一屏三列，grounded 框 5:6，不要上 1 下 2；第一屏 1440 核对卡后原位换成 full，点原图开灯箱看 full，预览和下载同一合成器，页头藏「下载 PPT」，灯光收到页头「调灯」后面；成片可切白底/银底/白桌白墙，当场合成不重打。没有地面图的旧单仍是上面三图看形、浅底+内描边、页头「下载 PPT」。缺印刷面时事故条在三图之上，可补则「补印刷面」。下面印刷面读字；每张图右上角下载，不提供 PDF 下载入口；截图时下载钮藏起来。界面 QA 时标出本次检查范围内与 `DESIGN.md` 不符的实现；范围外发现单列建议，不自动扩展修复。
