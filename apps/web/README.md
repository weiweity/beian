...
```text
ui/          React 审稿台 / 打样台 / 历史 / 设置（WaitCard；核对页 CSS transform 缩放 + rAF 拖动，不上 OpenSeadragon；核对窗浮在整页最上面含侧栏，进页展开叠在左侧栏；页图和工具进剩余区；窗不盖签字；收起有动画，边框八向缩放；打样台只交稿，点进度进打样单一屏三图；下载圆钮在图下说明行；打样台/打样单主区白底深字，GLB 中性环境；地址 `/` `/new` `/review/:id` `/mockup` `/mockup/:id` `/history` `/settings`；离开后看板/侧栏/历史记录仍显示阶段）
