...
审稿台（任务 / 新建 / 审核）、打样台、历史记录、设置（开工板 + 费用账单）。对照 / 对红 / 打样排队时用等待卡，不要假取消。离开后再进看板、打样台或历史记录，卡片、侧栏圆点和历史行仍显示阶段；点进行中的审稿进等待卡，打样进打样单。核对页有 bbox 才画真框，钉在框中心；「显示钉」和「显示框」分开，拖动画布时框暂时藏。初始 1× 页图不施加恒等 transform，避免高清 SVG 首屏被提前栅格化。核对窗液态玻璃 portal 到应用顶层，高于侧栏、页头、签字和画布；进页展开并叠在左侧栏上，可拖到视口任何位置，不挤页图。展开/收起按各自可见尺寸碰撞，收起后仍可拖到底边；边框上下左右和斜角都能缩放，窗内下拉弹层再高一层。疑点列表置顶；当前字段先 16px 嘱咐再结论下拉，Excel 应印与稿上读到两列。结论写在页头。打样台和打样单分开，点进度进打样单。有地面图的新单仍是一屏三列成片（不要上 1 下 2），预览和下载同一合成器，页头藏「下载 PPT」，灯光收到「调灯」后面；没有地面图的旧单仍是一屏三图看形、白底有描边、页头「下载 PPT」（没写成仍显示，点了出「PPT 没写成，白底仍可下」）。缺印刷面可补切。下面印刷面读字；产品/背景灯光分开调；打样中和失败可重试。唯一「上刀线」默认黑盒提交，失败对籽烨结案为打样失败，选层只给 admin。唯一可确认正面服务端自动出图；多面时桌面待选正面把展开图、选项、错误和主动作收在一屏。管理员可在已出图且只有一套公开盒型的单上换正面。打样台和打样单主区白底深字；每张图右上角下载；不提供 PDF 下载入口；点下载底部提示不挡操作；GLB 全屏居中；截图时下载钮藏起来；全屏被拒出「全屏打不开」；GLB 中性环境。地址：`/reviewup` `/reviewup/new` `/review/:id` `/mockup` `/mockup/new` `/mockup/:id` `/history` `/settings`，后退换台。旧 `/` `/new` `/review` 转到新地址。没有自建登录页：未登录会跳飞书授权。
