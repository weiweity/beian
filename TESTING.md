...
- [资源采证工具](scripts/resource-stress/README.md)：`python3 -B -m unittest discover -s scripts/resource-stress -p 'test_*.py' -v`。包含 R04 采样/失败/取消/清理、多轮聚合，以及 Q05 回执与停轮合同；只用自建轻量子进程、临时 Git 仓和合成 payload，不启动 Blender/浏览器压测，不构成独占预算或 Windows Job Object 证明。原 `test_harness.py -v` 仍可单跑 R04，但不能替代 Q05 工具回归。
