...
- [告警判断与 fixture 回放](scripts/monitoring/README.md)：`node --test scripts/monitoring/*.test.mjs scripts/monitoring/probes/*.test.mjs scripts/monitoring/delivery/*.test.mjs`。包含探测适配器、投递模块与串联合成回归；不访问生产或发送通知。
