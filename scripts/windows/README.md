...
RF-11 候选：`scripts/windows/blender-contract-smoke.ps1` 在 Illustrator Session 1 身份冒烟之后调用 `workers/packaging/tools/blender_contract_smoke.py`。输出只写 `RUNNER_TEMP`，复用 quality eval 与 `render_generation.compare_glb_artifact_contract` / hash 门，PowerShell 不复制 GLB 或 generation 检查。wrapper 默认 90 秒只是候选预算；`release.ps1` 当前传入 `-TimeoutMs 720000`，两者都未在杭州同机冻结。发版路径未解析到 `WB_BLENDER` / `BLENDER_EXECUTABLE` / `settings.json` 中的可执行文件时跳过、不回滚。独立脚本缺 `RUNNER_TEMP` / `WB_PYTHON`、超时、hash/GLB/generation 合同失败会非零退出；此时仍在 `Write-StartupDrainFence` 之后、`Commit-ReleaseRecovery` / `Open-TargetReleaseDrain` 之前，走 `Invoke-ArmedRecovery "升版失败"`。这不是 Windows 原生 Job Object 证明，也不是杭州已上线证据。
