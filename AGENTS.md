...
- 文档入口：`README.md`（完整文档地图、启动、设置）、`DESIGN.md`（视觉）、`docs/00-charter.md`（8/31 章程）、`docs/adr-004-ousterhout-design.md`（深模块）、`docs/adr-005-packaging-structure-v2.md`（打样结构）、`docs/designs/review-job-module.md`（对照/对红/打样入队）、`docs/designs/illustrator-unattended-control.md`（Illustrator 完工信号与禁杀阶段）、`docs/pouch-v1-acceptance.md`（膜袋 v1 标准，本轮不实现 3D）、`CHANGELOG.md`、`TODOS.md`。实现约定见下面「设计哲学」。
...
- 打样台 8/31 不对业务开放。打样台和打样单主区白底深字（不是紫雾台，深色主题也是）；有地面图的新单正面成片大约 480px 的 5:6 主图，预览和下载同一合成器，页头藏「下载 PPT」，灯光收到「调灯」后面；没有地面图的旧单仍是上面三图看形、浅底+内描边、页头「下载 PPT」。下面印刷面读字；每张图右上角下载，不提供 PDF 下载入口；截图时下载钮藏起来。QA 时标出任何与 `DESIGN.md` 不符的实现。
