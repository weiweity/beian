...
- 审稿：专属核对页，画布吃满宽度。OCR 真框始终在；「隐藏钉」只藏编号圆圈。左图 CSS transform 缩放（1–6×），拖动走 rAF，禁止原生图片拖拽，不上 OpenSeadragon。右上液态玻璃核对窗可收起、可缩放，字段和疑点两列。结论写在页头签字旁。禁止「AI 已过审」。
- 历史记录是侧栏 tab，不是第三张台。
