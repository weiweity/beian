...
- 打样台 8/31 不对业务开放。打样台和打样单主区白底深字（不是紫雾台，深色主题也是）；有地面图的新单走 grounded 成片（台面 `rgb(228,228,232)`），布局仍是一屏三列，grounded 框 5:6，不要上 1 下 2；第一屏 1440 核对卡、点原图换全图，预览和下载同一合成器，页头藏「下载 PPT」，灯光收到页头「调灯」后面；没有地面图的旧单仍是上面三图看形、浅底+内描边、页头「下载 PPT」。缺印刷面时事故条在三图之上，可补则「补印刷面」。下面印刷面读字；每张图右上角下载，不提供 PDF 下载入口；截图时下载钮藏起来。QA 时标出任何与 `DESIGN.md` 不符的实现。
