...
- 打样台 8/31 不对业务开放。打样台和打样单主区白底深字（不是紫雾台，深色主题也是）；打样单上面三图看形、下面印刷面读字；白底浅底+内描边，灯光滑条同时调静帧和 GLB 曝光；每张图右上角下载，页头「下载 PPT」且不提供 PDF 下载入口；截图时下载钮藏起来。QA 时标出任何与 `DESIGN.md` 不符的实现。
