...
- 打样台 8/31 不对业务开放。打样台和打样单主区白底深字（不是紫雾台，深色主题也是）；每张图右上角下载，页头「下载+PPT」。QA 时标出任何与 `DESIGN.md` 不符的实现。
