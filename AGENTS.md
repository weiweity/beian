...
- 文档入口：`README.md`（完整文档地图、启动、设置）、`DESIGN.md`（视觉）、`docs/00-charter.md`（8/31 章程）、`docs/adr-004-ousterhout-design.md`（深模块）、`docs/adr-005-packaging-structure-v2.md`（打样结构）、`docs/adr-007-packaging-render-fidelity.md`（3D 渲染真实感、材质、棚光、清晰度与能力矩阵；工程评审已锁定，尚未落代码）、`docs/designs/review-job-module.md`（对照/对红/打样入队）、`docs/designs/illustrator-unattended-control.md`（Illustrator 完工信号与禁杀阶段）、`docs/pouch-v1-acceptance.md`（膜袋 v1：L0 已接线为 3 mm 薄盒，无杭州金标不得宣称真实膜袋能力）、`CHANGELOG.md`、`TODOS.md`。实现约定见下面「设计哲学」。
