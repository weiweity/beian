...
| `workers/packaging/` | 2D→3D CLI，打样台调用。V2 以显式 `cut/crease/...` 语义为结构事实；确定性曲线适配、统一毫米坐标、连通分量隔离和有界拓扑只消费已确认的结构语义。无对象语义的旧稿先盘点与源稿绑定的纯描边候选层；只有管理员可在同一打样单选择 1–16 个候选图层，并在 5600px 原稿上看真实刀线/折线涂亮；籽烨看不到选层。烫雅银等工艺板与刀线分预算，坏层跳过不关整份刀线预览（旧单无快照则降级高清底图），由服务端复核源稿与候选成员后重跑识别，所选线稿仍须形成完整盒身环和上下封口才可进入 Blender。唯一可确认正面时服务端自动出图；多面时已登录账号在真实展开图上只点品名面。管理员可在已出图且只有一套公开盒型的单上换正面，只重跑 Blender。候选身份和几何推导的默认朝向仍作为内部锚点交给同一确认引擎，其余五面自动推导。不按颜色、零散矩形或 bbox 静默套盒。macOS 用 AppleScript，Windows 由登录桌面的 Illustrator Agent 经命名管道执行现有 VBS/JSX，服务进程不在 Session 0 拉起 Illustrator。平面出图用 pymupdf（对照同一 Python）；杭州不需要 macOS qlmanage。静帧磁盘保留 RGBA 产品层，并另出可选地面 pass（`front_right_ground` / `back_left_ground`，失败不挡产品图）；PPT 用两张白底写 OOXML；两张白底再合成一页 PDF。缺 PPT/PDF 仍算出图 |
| `docs/` | 章程、ADR、设计 |
...
| `docs/adr-007-packaging-render-fidelity.md` | 包装 3D 渲染真实感、材质、棚光、清晰度、能力矩阵与评测合同；RF-00–07 已合入 main（RF-07 为 `88321a90` / `0.21.41.0`，仅诊断 registry）；RF-08 投影采样已有诊断 profile 的 Code/L0；RF-09 浏览器分层升级已完成本地实现与合成验证；RF-10/RF-11 已合 `0490e46` / `0.21.44.0`（Code/普通 L0）。`0.21.45.0` 发版 L1 已过且杭州真跑 Blender 5.2 冒烟约 63s。`0.21.46.0` 把发版 `TimeoutMs` 冻成 90000，并用 Job Object 杀树。正式 baseline、质量门杭州实跑、L2/UAT 与生产注册见 [TODOS.md](TODOS.md) |
...
| `docs/designs/illustrator-unattended-control.md` | Illustrator 无人值守控制面：本代结果文件完工、禁杀存盘、busy 排队与外层 1260 秒 |
...
| `workers/packaging/README.md` | 打样 CLI、`PackagingStructure` 语义合同、完整盒型与正面锚点确认、pymupdf 平面出图、白底/印刷面/PPT/GLB 产物合同，以及 RF-00/RF-10 测量尺与三层质量结果、RF-11 Blender 合同冒烟、RF-02 同步回滚、RF-03 本地候选验证/资源生命周期，以及 RF-06/RF-07/RF-08 诊断 registry 边界 |
...
- 出图后点进打样单。有地面阴影的新单仍是一屏三列（正面+侧面、反面+侧面、GLB），grounded 框 5:6，不要上 1 下 2；下载和预览走同一套合成器；第一屏先出 1440 核对卡，各图层高清图就绪后原位替换；布景图慢时先显示可用产品和地面，卡坏了回退全图，高清图失败保留卡图并提示重新打开此单。点「原图」打开同页灯箱，继续加载全图。页头把「下载 PPT」藏起来，灯光收到「调灯」后面。可切白底 / 银底 / 白桌白墙，浏览器当场合成，不用重打样；有白台白墙静帧时白桌白墙用那张棚。没有地面图的旧单仍是上面三图看形，浅底+内描边，页头「下载 PPT」。缺正/反/左/右印刷面时事故条在三图之上；有已保存底稿可「补印刷面」，不重跑 3D。下面印刷面读字，可放到 6 倍。白底静帧是透明产品层再套白底；GLB 框不画内描边。产品灯光和背景灯光分开调，不用重打样；成片下载冻结点击时的背景和灯光，灯箱内点击按灯箱画面选择背景。已显示布景但布景高清图尚未就绪时提示重试；切换版本或离页后，已开始的下载仍可完成。打样中和打样失败可重试，不必重传。白底/成片下载仍只给正面+侧面 / 反面+侧面，不要用 GLB 读小字。旧单磁盘上已有各面印刷图时打开详情就能读，不必重跑。
- 网页新打样单固定进入语义结构 V2，不再回落到旧刀线猜测。籽烨只看 `正在出图`（打开稿件 / 盘点图层 / 保存 PDF，不编造 120 秒）→ `待选正面` / `打样失败` / `当前不支持` → `打样中`。没有完整花盒时看板/历史写「打样失败」，只有待选正面才写「待选正面」。有显式对象语义的 AI 会经曲线适配、统一坐标、连通分量隔离和拓扑预检发布完整“盒身环 + 封口组件”候选，并把真实 artwork 放在盒型图下；没有对象语义的旧 AI 会先列出与当前源稿绑定的纯描边候选层。唯一「上刀线」（或全单只剩一层「刀线」）默认连同候选里的「印刷」一起提交识别，失败对籽烨结案为打样失败，选层只给管理员；旁边还有「折线」的「刀线」仍要管理员勾。无确认权限或已有完整盒网时不会自动提交。选层涂亮时刀线与烫雅银等工艺板分开占用预览预算；坏掉的工艺板或对不上号的涂亮层会单独跳过，不会把整份刀线预览一起关掉。非管理员详情不返回图层候选。选层接口只给管理员；Hono 会复核管理员身份、源稿和候选成员，旧 `/1` 清单必须按当前稿件重新识别。所选线稿仍是不可信输入，只有完整盒身环与上下封口通过时才能继续。标准相向对开摇盖按两个物理成员的组合覆盖识别；原始 PDF 未绘制区和物理未覆盖区都保留 Alpha，在 Blender 中露出配置的纸板基底，不再要求单片覆盖整面或填白。未知基材默认白卡，牛皮纸/深色纸板必须显式配置；最终 GLB 会解码内嵌 PNG 核对 RGBA/Alpha 像素，并回读验证六面 UV 方向、镜像以及纸板 Core 的六个实体边界。外盒与内衬都成立时仍是不同整套候选、较大盒型在前，颜色不参与判断；页面只用“上一张 / 下一张”切换真实展开图，不展示尺寸、封口算法或 CAD 术语。唯一可确认正面时服务端自动出图，关页也继续。多面时已登录角色点印有品名和主视觉的那一面；预检引擎为每个正面写入几何推导的默认朝向，页面不再要求客户选择 0/90/180/270°。管理员可在已出图且只有一套公开盒型的单上换正面，只重跑 Blender。确认过期返回可刷新重选的 409，语义上不能成盒返回 422，并在原位显示，不当成网络中断。未确认、超出资源预算或严格复核失败时不会进 Blender。膜袋标准见 `docs/pouch-v1-acceptance.md`：`0.21.25.0` 已接线为 3 mm 薄直方体预览，不是鼓起、有封边或有插底的真实袋体；杭州金标前不得宣称膜袋产品能力。侧栏版本号来自已登录的 `/api/status.version`，紧跟在姓名下面，不在前端另写常量。合入后的杭州 Session 1 Agent → VBS → 唯一 JSX 身份冒烟是 L1 发布门；真实 Illustrator → Blender、六面贴图和管理员金标是独立 L2 业务验收，不是运行时开关。
- Windows 打样要求管理员处于已登录的交互桌面，登录任务 `beian-illustrator-agent` 会在 Session 1 建立受 ACL 保护的 UTF-8 命名管道。Hono 和 Actions runner 仍可留在 LocalSystem/Session 0，但只能通过该管道请求 Illustrator。心跳 busy 时新单排队，不当成代理掉线。用户注销、Agent 心跳过期或持久故障围栏尚未由交互管理员确认清除时，开始打样会在领取上传回执前返回可重试的 412，不会退回 Session 0 启动 Adobe。锁屏或 UU 断开不等于注销；恢复步骤见 `scripts/windows/README.md`。
...
| 角色 | 打样单 | 建单 | 确认结构 | 删除 |
|---|---|---|---|---|
| `admin` | 团队全部可读、可下载 | 可以 | 可以；另可选结构层 | 任意已结束单 |
| `reviewer` | 团队全部可读、可下载 | 可以 | 可以 | 仅本人已结束单 |
| `viewer` | 团队全部可读、可下载 | 不可以 | 可以 | 不可以 |
