...
- 出图后点进打样单。有地面阴影的新单：正面成片大约 480px 的 5:6 主图交差，下载和预览走同一套合成器；页头把「下载 PPT」藏起来，灯光收到「调灯」后面。没有地面图的旧单仍是上面三图（正面+侧面、反面+侧面、GLB）看形，浅底+内描边，页头「下载 PPT」。下面印刷面读字，可放到 6 倍。白底静帧是透明产品层再套白底；GLB 框不画内描边。产品灯光和背景灯光分开调，不用重打样；下载跟当前灯光走。打样中和打样失败可重试，不必重传。白底/成片下载仍只给正面+侧面 / 反面+侧面，不要用 GLB 读小字。旧单磁盘上已有各面印刷图时打开详情就能读，不必重跑。
