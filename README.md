...
- 出图后点进打样单。有地面阴影的新单仍是一屏三列（正面+侧面、反面+侧面、GLB），grounded 框 5:6，不要上 1 下 2；下载和预览走同一套合成器；第一屏先出 1440 核对卡，点「原图」再换全图，卡坏了回退全图。页头把「下载 PPT」藏起来，灯光收到「调灯」后面。没有地面图的旧单仍是上面三图看形，浅底+内描边，页头「下载 PPT」。缺正/反/左/右印刷面时事故条在三图之上；有已保存底稿可「补印刷面」，不重跑 3D。下面印刷面读字，可放到 6 倍。白底静帧是透明产品层再套白底；GLB 框不画内描边。产品灯光和背景灯光分开调，不用重打样；下载跟当前灯光走。打样中和打样失败可重试，不必重传。白底/成片下载仍只给正面+侧面 / 反面+侧面，不要用 GLB 读小字。旧单磁盘上已有各面印刷图时打开详情就能读，不必重跑。
