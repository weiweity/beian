...
- 出图后点进打样单：上面三图（正面+侧面、反面+侧面、GLB）只看形；下面印刷面读字，可放到 6 倍。白底静帧用浅底+内描边，白盒才看得见边；GLB 框不画内描边。页头「产品灯光」和「背景灯光」分开调，不用重打样；下载白底跟当前灯光走。打样中和打样失败可重试，不必重传。白底下载仍只给正面+侧面 / 反面+侧面，不要用 GLB 读小字。旧单磁盘上已有各面印刷图时打开详情就能读，不必重跑。
