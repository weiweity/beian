...
- 出图后点进打样单。有地面阴影的新单仍是一屏三列（正面+侧面、反面+侧面、GLB），grounded 框 5:6，不要上 1 下 2；下载和预览走同一套合成器；第一屏先出 1440 核对卡，各图层高清图就绪后原位替换；布景图慢时先显示可用产品和地面，卡坏了回退全图，高清图失败保留卡图并提示重新打开此单。点「原图」打开同页灯箱，继续加载全图。页头把「下载 PPT」藏起来，灯光收到「调灯」后面。可切白底 / 银底 / 白桌白墙，浏览器当场合成，不用重打样；有白台白墙静帧时白桌白墙用那张棚。没有地面图的旧单仍是上面三图看形，浅底+内描边，页头「下载 PPT」。缺正/反/左/右印刷面时事故条在三图之上；有已保存底稿可「补印刷面」，不重跑 3D；提交后排队，刷新或离页仍继续，成片保持可见，失败可重试。下面印刷面读字，可放到 6 倍。白底静帧是透明产品层再套白底；GLB 框不画内描边。产品灯光和背景灯光分开调，不用重打样；成片下载冻结点击时的背景和灯光，灯箱内点击按灯箱画面选择背景。已显示布景但布景高清图尚未就绪时提示重试；切换版本或离页后，已开始的下载仍可完成。打样中和打样失败可重试，不必重传。白底/成片下载仍只给正面+侧面 / 反面+侧面，不要用 GLB 读小字。旧单磁盘上已有各面印刷图时打开详情就能读，不必重跑。 GLB 默认以正面、侧面和少量顶面展示，旋转或缩放后可点「复位」恢复，切换背景保留当前视角。白桌白墙仅供查看，下载仍为原始包装 GLB；墙后观察与调灯边界见 [GLB 查看器显示合同](docs/contracts/packaging-presentation.md#glb-查看器显示)。
