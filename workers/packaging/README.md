...
这是面向重复包装结构的本地批处理流程。网页打样先读稿上 `刀线` / `刀版` 还原盒面（尺寸不预置）；没有刀线才回退 `templates/` 已登记 JSON（含 47.5 × 47.5 × 177.5 mm 方形花盒）。目标是在 AI 稿结构不变时，自动完成：
...
- 相同刀模只需新增任务记录即可并行处理。
- 网页打样先读 `刀线` / `刀版` 层，从展开图还原长宽高和切面（花盒或膜袋，尺寸不预置）。没有刀线层才回退到 `templates/` 里已登记 JSON。读不出结构时失败，不要把方盒硬套到扁平盒或膜袋。
...
- .glb：可旋转查看的通用 3D 文件；
- 正面/右侧面和背面/左侧面白底渲染图；
- 独立 .pptx（一页两张白底：正面+侧面、反面+侧面；跳过 PPT 时没有这一项，也不当失败）；
- qa/：PPT 质检图、蒙太奇、布局检查和来源记录（跳过 PPT 时没有）；
