...
独立冒烟只证明合成 carton 能走过 quality eval 与 generation GLB/hash 门，不证明真实感，也不替代发版 L1。输出必须落在 `RUNNER_TEMP` 下，不读客户任务、不上传、不切产品 current。PowerShell 只编排超时与环境变量，不复制 GLB 校验。杭州发版跳过冒烟时，质量门杭州实跑仍开放，见 [TODOS.md](../../TODOS.md)。
