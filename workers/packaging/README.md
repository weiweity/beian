...
`tools/render_quality_eval.py` 用固定的合成夹具观测当前包装 3D 输出，不改变 `pipeline.py`、Blender 场景或产品图片。RF-10 在同一 CLI 上输出三层独立结果：runtime hard（产物完整性与资源合同）、fixture regression hard（夹具身份与基线回归）、warning/human（视觉观察与 `human_acceptance`）。机器通过不会写成人工通过，`production_ready` 保持 false。`baseline_mismatch` 只失败夹具回归门，不表示真实任务视觉不合格。清单包含 6 个已支持的矩形盒场景（白盒、深色盒、高盒、宽盒、字体频率和透明边缘）以及 1 个必须明确判为不支持的圆柱场景；结果不能代替杭州 Windows L1、真实稿 L2 或人工 UAT。
...
独立冒烟只证明合成 carton 能走过 quality eval 与 generation GLB/hash 门，不证明真实感或杭州 L1。输出必须落在 `RUNNER_TEMP` 下，不读客户任务、不上传、不切产品 current。PowerShell 只编排超时与环境变量，不复制 GLB 校验。

    RUNNER_TEMP=/tmp/beian-rf11-smoke \
    apps/web/backend/.venv/bin/python workers/packaging/tools/blender_contract_smoke.py \
      --blender /absolute/path/to/blender

Windows 入口是 `scripts/windows/blender-contract-smoke.ps1`。wrapper 默认 90 秒只是候选预算；`release.ps1` 当前传入 `TimeoutMs 720000`，两者都未在杭州冻结。缺 `RUNNER_TEMP` / 已解析 Blender 时：独立脚本失败关闭，发版路径跳过且不回滚。stdout JSON 含 `quality_layers`，`human_acceptance` 保持 pending，`production_ready` 保持 false。
