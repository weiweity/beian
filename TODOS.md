...
- [ ] **膜袋 v1 3D**
  标准已写在 `docs/pouch-v1-acceptance.md`：文件名/标题/图层名带「膜袋」才分流，两块相近闭合轮廓，六面贴进现有 `add_box` 薄盒。本轮 0.21.19 只交文档。接线前要有杭州金标正/负各一份，不要复活 `dieline.py` 栅格猜测，不要把 12%/3mm 写进章程。hold 名单仍是 膜袋/内包/标贴；袋装花盒当花盒。
...
- [ ] **真桌子真墙金标重渲**
  只有实机否决接触影/遮挡/反射，才建低优先级、可去重、不改产品 RGBA 的 Blender set/ground 衍生任务。无杭州金标静帧不给新几何按钮。银底没拒图证据不做真金属地。木桌仍 hold。
