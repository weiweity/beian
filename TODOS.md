...
主计划：`docs/adr-007-packaging-render-fidelity.md`。本分支已完成 `/plan-eng-review` 并锁定 D1–D8；尚未合入、未落实现，不得把计划参数直接写进生产。

- [ ] **锁定 ADR-007 与执行切片**
  明确现有流水线内部重构边界、`packaging-render-spec/1`、能力矩阵、兼容策略、质量门与 Grok 交付顺序。保留 Hono、作业队列、`ResolvedPackagingJob`、产物 key 和同一 Blender 后台进程，不另建服务或第二套 3D 流水线。

- [ ] **建立渲染基线与可复现评测**
  用 Git 内脱敏合成样片记录当前主线的几何、白盒边界、纸材、字体边缘、Alpha 光边、渲染耗时和首屏/原图差异；私有真稿只在杭州记录哈希、评分和结论。先留基线，再改参数。

- [ ] **版本化渲染合同与能力矩阵**
  在结构确认后、切面栅格前，由 `workers/packaging` 内部解析 family、几何、纸材、涂层、棚光、相机、采样和输出配置；合同进入缓存指纹和 `pipeline_result.json`。未知 family / profile 失败关闭，不按文件名、颜色或任务 ID 猜模型。
